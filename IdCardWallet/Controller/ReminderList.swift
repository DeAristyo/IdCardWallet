...
class ReminderList: UIViewController {
    ...
    @objc private func openModal() {
        let modalView = ReminderSheet(title: "Add Reminder")
        let navigationController = UINavigationController(rootViewController: modalView)
...
extension ReminderList: UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: CustomTableCell.identifier, for: indexPath)
                        as? CustomTableCell
